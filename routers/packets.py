...
@router.post("/leagues/{league_id}/packets", summary="Upload Packet")
async def upload_packet(
    league_id: str,
    request: Request,
    file: UploadFile = File(..., description="Packet file to upload (e.g., 555B0201.001)"),
    client: Client = Depends(get_current_client),
    db: Session = Depends(get_db),
):
    """
    Upload a game packet to the hub

    **Path Parameters:**
    - `league_id`: Three-digit league number (e.g., "555")

    **Form Data:**
    - `file`: The packet file to upload
    ...
    **Example:**
    ```bash
    curl -X POST "https://hub.example.com/api/v1/leagues/555/packets" \\
...
async def list_packets(
    ...
    **Example:**
    ```bash
    # Get all packets
    curl -X GET "https://hub.example.com/api/v1/leagues/555/packets" \\
      -H "Authorization: Bearer YOUR_TOKEN"

    # Get only unread packets
    curl -X GET "https://hub.example.com/api/v1/leagues/555/packets?unread=true" \\
      -H "Authorization: Bearer YOUR_TOKEN"
    ```
    """
...
async def download_nodelist(
    ...
    **Example:**
    ```bash
    curl -X GET "https://hub.example.com/api/v1/leagues/555/nodelist" \\
      -H "Authorization: Bearer YOUR_TOKEN" \\
      -o BRNODES.555
    ```
    """
...
async def download_packet(
    ...
    **Example:**
    ```bash
    curl -X GET "https://hub.example.com/api/v1/leagues/555/packets/555B0102.001" \\
      -H "Authorization: Bearer YOUR_TOKEN" \\
      -o 555B0102.001
    ```
    """
