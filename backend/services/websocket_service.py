...
_dashboard_connections: Set[WebSocket] = set()  # Management dashboard connections
...
# --- Dashboard WebSocket (Management API) ---


async def connect_dashboard(websocket: WebSocket):
    """Register a new dashboard WebSocket connection"""
    await websocket.accept()
    _dashboard_connections.add(websocket)
    logger.debug(f"Dashboard connected (total: {len(_dashboard_connections)})")


async def disconnect_dashboard(websocket: WebSocket):
    """Remove a dashboard WebSocket connection"""
    _dashboard_connections.discard(websocket)
    logger.debug(f"Dashboard disconnected (total: {len(_dashboard_connections)})")
